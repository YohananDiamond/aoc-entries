...
struct Dimension {
    ...
    /// FIXME: this might be inefficient; maybe a Vec<Vec<>> with some kind of offset would be more efficient? I don't know.
...
fn part1(input: &str) -> Result<String, String> {
    let mut dimension = Dimension::with_initial_grid(input)?;
...
impl Dimension {
    ...
    pub fn perform_cycle(&mut self) {
        ...
        for active_cube_pos in self.active_cubes.iter() {
            ...
            // let nearby_active_count = (-1..=1).fold(0, |result, y| {
            //     result
            //         + (-1..=1).fold(0, |result, x| {
            //             result
            //                 + (-1..=1).fold(0, |result, z| {
            //                     let nearby_pos = Point3 { x, y, z };

            //                     if nearby_pos != *active_cube_pos
            //                         && self.active_cubes.contains(&nearby_pos)
            //                     {
            //                         result + 1
            //                     } else {
            //                         result
